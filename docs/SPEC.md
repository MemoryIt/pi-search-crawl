...
```json
{
  ...
  "llm": {
    "clean": {
      ...
      "systemPrompt": {
        "type": "file",
        "value": "prompts/clean.md"
      }
    },
    "summary": {
      "providerName": "litellm",
      "modelName": "omlx/gemma-4-E2B-it-MLX-8bit",
      "systemPrompt": {
        "type": "string",
        "value": "请用中文总结以下内容，控制在 300 字以内..."
      }
    }
  },
  "storage": {
...
> **注意**：API Key 的获取不再由 config.json 管理，而是通过 `ModelRegistry.getApiKeyAndHeaders()` 统一从 Pi 平台的 `models.json`、`auth.json` 和环境变量中获取。优先级：`models.json (providerConfig.apiKey)` > `auth.json` > 环境变量。
...
🔍 [clean] 检查本地缓存...
🧹 调用清洗模型 (litellm/omlx/Qwen3.5-0.8B-MLX-8bit)...
✅ 清洗完成 (clean.md: 8.1 KB)
☁️ 上传 clean 到 S3...
🔍 [summary] 检查本地缓存...
📝 调用总结模型 (litellm/omlx/gemma-4-E2B-it-MLX-8bit)...
...
├── llm.ts           # LLM 调用（通过 pi-ai + ModelRegistry 调用 Pi 已配置的模型）
├── package.json
└── docs/
    ├── README.md
    ├── ARCHITECTURE.md
...
```typescript
// - Config 类型定义（LLMConfig 使用 providerName + modelName 选择模型）
// - 默认配置
// - config.json 加载逻辑
// - systemPrompt 解析（string/env/file）
// - Schema 校验
```
...
```typescript
// - AuthStorage 单例（读取 Pi 的 auth.json 和环境变量）
// - ModelRegistry 单例（加载 Pi 的 models.json 配置）
// - callLLM(config, content) - 核心调用函数
//   1. 从 ModelRegistry.find(providerName, modelName) 查找模型
//   2. 通过 registry.getApiKeyAndHeaders(model) 获取 API Key
//      （优先级：models.json > auth.json > 环境变量）
//   3. 调用 completeSimple(model, context, options) 执行 LLM 请求
//   4. 从 AssistantMessage 提取纯文本内容
// - callCleanLLM(config, content) → LLMResult
// - callSummaryLLM(config, content) → LLMResult
// - getLLMInfo(config) → { providerName, modelName }
```
...
```json
{
  "dependencies": {
    ...
    "@mariozechner/pi-ai": "^x.x",
    "@mariozechner/pi-coding-agent": "^x.x",
