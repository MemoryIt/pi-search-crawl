...
- [ ] 创建 config.ts，分离配置逻辑
- [ ] 实现 config.json 加载和校验
- [ ] 实现 systemPrompt 解析（string/env/file）
- [ ] 实现 S3 客户端和存储操作
- [ ] 实现 LLM 调用函数
- [ ] 实现缓存逻辑（本地 + S3）
- [ ] 实现 fetch_web_page 工具（含进度输出）
- [ ] 更新 search 工具（如需要）
- [ ] 编写单元测试
- [ ] 更新文档
...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| 2026-05-03 | v0.2.0 | 初始版本：新增 config.json 配置、LLM 清洗/总结、S3 存储、缓存逻辑 |
| - | v0.1.0 | 初始版本：search 和 fetch_web_page 基础功能 |
